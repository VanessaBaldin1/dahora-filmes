...
export async function salvarFilmeFavorito(filme: Filme): Promise<boolean> {
  ...
  if (favoritos.some((filmeExistente) => filmeExistente.id === filme.id)) {
    ...
    // Por isso, retornamos false para indicar que o filme NÂO FOI adicionado
    return false;
  }
...
export async function removerFilmeFavorito(id: number): Promise<void> {
  ...
  //Filtrando a lista de favoritos já existente, avaliando qual filme deve ser "descartado/removido". Com isso, geramos uma nova lista atualizada SEM o Filme que deve ser removido
